...
describe('OpenAI provider', () => {
  describe('translateBatch', () => {
    it('translates a batch of messages', async () => {
      ...
      const results = await provider.translateBatch(REQS, 'fake-key', 'gpt-4o-mini', 'zh-TW')
    ...
    it('returns error for non-ok response', async () => {
      const fetchFn = mockFetch(401, { error: { message: 'Incorrect API key' } })
      const provider = createOpenAIProvider(fetchFn)

      const results = await provider.translateBatch(REQS, 'bad-key', 'gpt-4o-mini', 'zh-TW')
    ...
    it('returns error when choices are missing', async () => {
      ...
      expect(results[0]!.error).toBe('Empty response from OpenAI')
    })
  ...
  describe('validateKey', () => {
    it('validates a correct key', async () => {
      const fetchFn = mockFetch(200, { data: [{ id: 'gpt-4o-mini' }] })
      const provider = createOpenAIProvider(fetchFn)
